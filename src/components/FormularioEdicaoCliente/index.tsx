...
import { z } from "zod";
import useUpdateClient from "../../Hooks/EditarCliente";
...
const FormularioEdicaoCliente: React.FC<FormularioEdicaoClienteProps> = ({
  ...
  useEffect(() => {
    if (clientData) {
      setValue("nome", clientData.nome);
      setValue("nomeSocial", clientData.nomeSocial);
      setValue("email", clientData.email);
      setValue("endereco", clientData.endereco);
      clientData.telefones.forEach((telefone: any, index: number) => {
        setValue(`telefones.${index}`, telefone);
      });
    }
  }, [clientData, setValue]);
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {telefoneFields.map((field, index) => (
            <div key={field.id} className="flex mb-2">
              ...
              />
              {errors.telefones?.[index]?.ddd && <p className="text-red-500 text-xs">{errors.telefones[index].ddd.message}</p>}
              ...
              />
              {errors.telefones?.[index]?.numero && <p className="text-red-500 text-xs">{errors.telefones[index].numero.message}</p>}
            </div>
          ))}
