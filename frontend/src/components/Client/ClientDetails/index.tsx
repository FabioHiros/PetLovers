...
import useFetchClientById from "../../../Hooks/ClientHooks/BuscaClientePorID";
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-lg relative">
        ...
        <h2 className="text-2xl font-semibold">Nome: {client.name}</h2>
        <p>Nome Social: {client.nomeSocial}</p>
        <p>CPF: {client.cpf}</p>
        <p>RG: {client.cpf}</p>
        
        <h3 className="mt-4 font-semibold">Telefones:</h3>
        {client.phones.map((telefone: any) => (
          <p key={telefone.id} className="text-sm text-gray-700">
            ({telefone.ddd}) {telefone.numero}
          </p>
        ))}
        <h3 className="mt-4 font-semibold">Pets:</h3>
        {client.pets.map((pet:any)=>(
          <p key={pet.id} className="text-sm text-gray-700">
            Nome: {pet.name} | Tipo: {pet.type} | Raça: {pet.breed}
          </p>
        ))}
